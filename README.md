...
- **无强推、无广告**：只展示你启用的源与类别，不做热度或算法强推；可选的「AI 精选」须主动触发且不改变时间线
...
- **AI 智读（可选）**：自备 OpenAI 兼容接口即可启用文章 AI 解读（摘要 / 要点 / 情绪）、按本地阅读记录生成的「AI 精选」、AI 助手问答与查找新闻、以及「舆情：企业名」舆情报告；Key 只存本机，检索只用本机缓存，不改变默认时间线
...
全部 AI 能力都是**可选**的：在「我的 → AI 智读设置」填入自己的 OpenAI 兼容接口（Base URL / API Key / Model，可从 AI 翻译一键导入）后即可使用；不配置则完全不影响原有阅读体验。

- **文章解读**：阅读器顶栏「解读」，生成两三句摘要、要点列表、主题标签与情绪倾向；结果缓存本机，可重新生成
- **AI 精选**：首页顶栏「精选」，把当前分类的候选文章连同本机「最近阅读 / 稍后读」的标题级画像发给你的接口，挑出感兴趣的文章并给出理由；只在独立页面展示，**不改变时间线排序**
- **AI 助手**：「我的 → AI 助手」，基于全部信源的本地列表缓存做检索问答，回答标注引用报道、可点回原文；也能查找感兴趣的新闻
- **企业舆情**：在助手中输入「舆情：企业名」。不是简单搜一遍公司名，而是先让 AI 把主体扩展成一组检索词（别名、子公司、产品、高管、所属行业与竞对），再跨板块跨信源把相关报道**整体召回**，分批归纳后合成报告（总体倾向 / 正面动态 / 负面与风险 / 板块与行业背景 / 关注建议），并注明本次分析覆盖了多少篇报道、多少信源、哪些板块

隐私边界与 AI 翻译一致：请求直连你配置的服务地址，应用作者不中转、无法查看；发送内容仅限当次所需的文章文本与标题级阅读画像。
