...
# === 路由审议 (v9.9.6) ===
route_confidence: 0.9             # 0-1, 主 agent 路由决策摘要中的置信度
route_history:                    # re-route 记录, 最多 10 条; 详情归 sprint/session-log
  ...
  - "2026-08-24 Feature/impl reading-preference: 代码已 push origin (dev=c903fbc, release=00aff2f merge-both); 未跑 PACE generator/review, 不宣称 ship (c=0.9)"
  - "2026-08-24 idle: 停 Vite、完善 .ai_state; path/stage/slug 清空以免假 ship 触发 delivery-gate (c=0.95)"
...
- 2026-08-24: 阅读偏好已合入 git：主题分类+出品方、精选 70/30、画像每 4 小时、模型配置在 AI 助手内。Vite :5173 已停。
- 2026-08-24: origin `main=aace5ba` (upstream 1.6.4)、`dev=c903fbc`、`release=00aff2f` (merge-both)。档案 `sprints/2026-08-24-reading-preference/`。
- 2026-08-24: 未跑 PACE generator/review；路由 idle（path/stage/slug 空）。不宣称 ship。下次新任务再路由。
